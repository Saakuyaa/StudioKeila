...
// Inserir dados no banco quando o formulário é enviado
$mensagem = '';
$agendamentos = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($diaDaSemana == 0) {
        ...
        if ($horaFormatada < '08:00' || $horaFormatada > '18:00') {
            ...
            $sqlVerificacao = "SELECT * FROM agendamentos WHERE data = '$data' AND hora = '$hora'";
...
// Recuperar todos os agendamentos após inserir
$resultado = $conn->query("SELECT * FROM agendamentos");
while ($row = $resultado->fetch_assoc()) {
    $row['data'] = date('d/m/Y', strtotime($row['data'])); // Formatar a data
    $agendamentos[] = $row;
}
    ...
    <?php if (!empty($agendamentos)): ?>
        ...
        <ul id="lista-agendamentos">
            <?php foreach ($agendamentos as $row): ?>
                <li><?php echo $row['nome'] . " - " . $row['telefone'] . " - " . $row['servico'] . " - " . $row['data'] . " - " . $row['hora']; ?></li>
            <?php endforeach; ?>
        </ul>
    <?php endif; ?>
</body>
